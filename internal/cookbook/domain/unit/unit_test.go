...
func TestUnitNewUnit(t *testing.T) {
	...
	for name, testCase := range testCases {
		...
		t.Run(name, func(s *testing.T) {
			...
			newUnit, err := unit.NewUnit(testCase.id, testCase.name, testCase.ownerID, testCase.timestamp)
			if err == nil {
				assert.Equal(t, testCase.id, newUnit.ID(), "different ID")
				assert.Equal(t, testCase.timestamp.UTC(), newUnit.CreatedAt(), "different CreatedAt")
...
func TestUnitUpdate(t *testing.T) {
	...
	newUnit, err := unit.NewUnit(uuid.New(), "custom unit", uuid.New(), time.Now())
	if err != nil {
